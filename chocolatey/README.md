# Chocolatey package for DiskGeek

The package source for `diskgeek` on the Chocolatey community repository lives here so that
`packageSourceUrl` in the nuspec points at something real.

## What the package does

It downloads `DiskGeekSetup.exe` from the GitHub release for the matching tag and verifies it against a
...
2. Take the SHA-256 for `DiskGeekSetup.exe` from the release's own `SHA256SUMS.txt`.
...
5. Install locally from the built nupkg and check the shim works:
   `choco install diskgeek -s . -y`
6. `choco push diskgeek.<version>.nupkg --source https://push.chocolatey.org/`
