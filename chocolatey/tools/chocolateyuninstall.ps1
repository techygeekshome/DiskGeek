...
$key = Get-UninstallRegistryKey -SoftwareName 'DiskGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'DiskGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for DiskGeek. Remove it manually rather than guessing:"
